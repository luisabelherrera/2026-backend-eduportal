...
@Autowired
    ...
    public UserDto updateUser(Long id, @Valid RegisterDto updateDto) throws NotFoundException, ConflictException {
        ...
        // Manejar Estudiante
        if (updateDto.getEstudianteId() != null) {
            Estudiante estudiante = estudianteRepository.findById(updateDto.getEstudianteId())
                    .orElseThrow(() -> new NotFoundException("¡Estudiante no encontrado!"));
        ...
        if (updateDto.getDocenteId() != null) {
            Docente docente = docenteRepository.findById(updateDto.getDocenteId())
                    .orElseThrow(() -> new NotFoundException("¡Docente no encontrado!"));
